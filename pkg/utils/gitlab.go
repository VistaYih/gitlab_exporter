...
func NewGitlabClient(config string) (*Client, error) {
	if config == "" {
	...
	cfg1 := &Config{}
	if err := cfg1.Parse(config); err != nil {
		return nil, err
	}

	// Configure GitLab client
	httpTransport := &http.Transport{
		TLSClientConfig: &tls.Config{InsecureSkipVerify: cfg1.SkipTLSVerify},
	}

	c := &Client{
		gitlab.NewClient(&http.Client{Transport: httpTransport}, cfg1.Token),
	}
	if err := c.SetBaseURL(cfg1.URL); err != nil {
		return nil, err
	}
